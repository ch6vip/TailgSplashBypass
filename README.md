...
- 可在主车 TBox 设置中补充“电池信息”快捷入口，按当前车型沿用官方分流，自动打开普通、C39、TLV 或 BMS 电池页；无法安全判断的非 GPS 车型不会误跳转。
...
- 五项功能互相独立且默认关闭，不会全局改写 `DeviceFunction`，也不会伪造 ECU/BLE 能力或发送自定义控车指令。
...
- 默认关闭：极速启动、蓝牙连接恢复、Bugly 屏蔽、首页导航精简、爱车与服务互换、月度骑行数据、制动力数据、电池信息快捷入口、电池动态入口、自定义车辆音效、感应距离覆盖、详细日志。
...
当前版本由 `gradle.properties` 唯一管理：`moduleVersionCode=10901`、`moduleVersionName=v1.9.1`。
